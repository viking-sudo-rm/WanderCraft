...
public class EntityItem extends Entity
{
    ...
    public void onCollideWithPlayer(EntityPlayer entityplayer)
    {
        ...
        if(delayBeforeCanPickup == 0 && entityplayer.inventory.addItemStackToInventory(item))
        {
			...
			//SnorriDev block aquire achievements
			if (item.itemID == Item.redlightsaber.shiftedIndex) {
				entityplayer.triggerAchievement(AchievementList.youAreNotMyFather);
			}
			if (item.itemID == Item.helmetChain.shiftedIndex) {
				entityplayer.triggerAchievement(AchievementList.likeFatherLikeSon);
			}
			if (item.itemID == Item.blasterpistol.shiftedIndex) {
				entityplayer.triggerAchievement(AchievementList.shootEmUp);
			}
			//SnorriDev
